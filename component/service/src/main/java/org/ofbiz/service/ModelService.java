...
public class ModelService extends AbstractMap<String, Object> implements Serializable {
  ...
  /**
   * Override the default transaction timeout, only works if we start the
   * transaction
   */
  public int transactionTimeout;

  /**
   * Sets the max number of times this service will retry when failed (persisted
   * async only)
   */
  ...
  /**
   * Context Information, a Map of parameters used by the service, contains
   * ModelParam objects
   */
  protected Map<String, ModelParam> contextInfo = FastMap.newInstance();

  /**
   * Context Information, a List of parameters used by the service, contains
   * ModelParam objects
   */
  protected List<ModelParam> contextParamList = FastList.newInstance();

  /**
   * Flag to say if we have pulled in our addition parameters from our
  ...
  /*
   * DEJ20060125 This is private but not used locally, so just commenting it out
   * for now... may remove later private void copyParams(Collection params) { if
   * (params != null) { Iterator i = params.iterator(); while (i.hasNext()) {
   * ModelParam param = (ModelParam) i.next(); addParam(param); } } }
   */
      ...
      for (ModelParam param : params) {
        if ("INOUT".equals(param.mode) || mode.equals(param.mode)) {
          ...
          if (defaultValueObj != null && context.get(param.name) == null) {
            ...
            Debug.logInfo("Set default value [" + defaultValueObj + "] for parameter ["
                + param.name + "]", module);
          }
        }
      }
  ...
  public void validate(Map<String, Object> context, String mode, Locale locale) throws ServiceValidationException {
    ...
    if (verboseOn)
      Debug.logVerbose("[ModelService.validate] : {" + this.name + "} : Validating context - "
          + context, module);
    ...
    // check for requiredButNull fields and return an error since null values
    // are not allowed for required fields
      ...
      for (String missingKey : requiredButNull) {
        ...
        if (message == null) {
          String errMsg = UtilProperties.getMessage(ServiceUtil.resource,
                                                    "ModelService.following_required_parameter_missing",
                                                    locale);
    ...
    try {
      ...
      Debug.logError("[ModelService.validate] : {" + name + "} : (" + mode
          + ") Required test error: " + e.toString(), module);
      throw e;
    }
    ...
    if ("IN".equals(mode)) {
      ...
      for (ModelParam modelParam : this.contextInfo.values()) {
        if (context.get(modelParam.name) != null
          ...
          // the param is a String, allow-html is none or safe, and we are
   ...
   */
  public static void validate(Map<String, String> info,
                              Map<String, ? extends Object> test,
                              boolean reverse,
                              ModelService model,
                              String mode,
    ...
    for (String key : testSet) {
      ...
      if (UtilValidate.isNotEmpty(param.validators)) {
        ...
        if (!ObjectType.instanceOf(testObject, infoType, null)) {
          ...
          String msg = "Type check failed for field [" + model.name + "." + key
              + "]; expected type is [" + infoType + "]; actual type is [" + testType + "]";
          typeFailMsgs.add(msg);
        }
      }
    }
  ...
  public static boolean typeValidate(ModelParam.ModelParamValidator vali, Object testValue) throws GeneralException {
    ...
    if (validatorMethod == null) {
      throw new GeneralException("Unable to find validation method [" + vali.getMethodName()
          + "] in class [" + vali.getClassName() + "]");
    }
    ...
    try {
      ...
      throw new GeneralException("Validation method [" + vali.getMethodName() + "] in class ["
          + vali.getClassName() + "] did not return expected Boolean");
    } catch (Exception e) {
      throw new GeneralException("Unable to run validation method [" + vali.getMethodName()
          + "] in class [" + vali.getClassName() + "]");
    }
   ...
   * parameters will be returned in the order specified in the file. Note: IN
   * and OUT will also contains INOUT parameters.
   * 
   ...
   */
  public Map<String, Object> makeValid(Map<String, ? extends Object> source,
                                       String mode,
  ...
  public Map<String, Object> makeValid(Map<String, ? extends Object> source,
    ...
    if (locale == null) {
      // if statement here to avoid warning messages for Entity ECA service
      // input validation, even though less efficient that doing a straight get
        ...
        if (UtilValidate.isNotEmpty(param.stringMapPrefix) && !source.containsKey(key)) {
          ...
          if (source.containsKey(key)) {
            if ((param.internal && includeInternal) || (!param.internal)) {
              ...
              try {
                // no need to fail on type conversion; the validator will catch
                // this
                ...
                String errMsg = "Type conversion of field [" + key + "] to type [" + param.type
                    + "] failed for value \"" + value + "\": " + e.toString();
   ...
   */
  public Map<String, Object> evalPermission(DispatchContext dctx,
    ...
    if (UtilValidate.isNotEmpty(this.permissionServiceName)) {
      ...
      if (permission != null) {
        ...
        Map<String, Object> result = ServiceUtil.returnSuccess();
        result.put("hasPermission", Boolean.FALSE);
        result.put("failMessage", "No ModelService found with the name ["
            + this.permissionServiceName + "]");
        return result;
      }
    } else {
      Map<String, Object> result = ServiceUtil.returnSuccess();
   ...
   * 
   * @return A list of required IN parameters in the order which they were
   *         defined.
   */
      ...
      if (this.engineName.equals("group") && implServices.size() == 0) {
        ...
        if (group != null) {
          for (GroupServiceModel sm : group.getServices()) {
            ...
            if (Debug.verboseOn())
              Debug.logVerbose("Adding service [" + sm.getName() + "] as interface of: ["
                  + this.name + "]", module);
          }
        }
      }
          ...
          if (model != null) {
            for (ModelParam newParam : model.contextParamList) {
              ...
              if (existingParam != null) {
                // if the existing param is not INOUT and the newParam.mode is
                // different from existingParam.mode, make the existing param
                // optional and INOUT
                // TODO: this is another case where having different
                // optional/required settings for IN and OUT would be quite
                // valuable...
                ...
                if (optional) {
                  // default option is to make this optional, however the
                  // service can override and
            ...
            Debug.logWarning("Inherited model [" + serviceName + "] not found for [" + this.name
                + "]", module);
          }
        }
      }
      ...
      if (UtilValidate.isNotEmpty(overrideParameters)) {
        for (ModelParam overrideParam : overrideParameters) {
          ...
          if (existingParam != null) {
            ...
            Debug.logWarning("Override param found but no parameter existing; ignoring: "
                + overrideParam.name, module);
          }
        }
      }
  ...
  public void getWSDL(Definition def, String locationURI) throws WSDLException {
    ...
    // soapAction should be set to the location of the SOAP URI, or Visual
    // Studio won't construct the correct SOAP message
    soapOperation.setSoapActionURI(locationURI);
    // this is the RPC/literal style. See
