...
public class ServiceDispatcher {
  ...
  protected ServiceDispatcher(Delegator delegator,
    ...
    if (delegator != null) {
      try {
        ...
        Debug.logError(e,
                       "[ServiceDispatcher.init] : No instance of security implementation found.",
                       module);
      }
    }
    ...
    serviceDebugMode = "true".equals(UtilProperties.getPropertyValue("service",
                                                                     "servicedispatcher.servicedebugmode",
                                                                     "true"));
  }
  ...
  /**
   * Returns a pre-registered instance of the ServiceDispatcher associated with
   * this delegator.
   * 
  ...
  public static ServiceDispatcher getInstance(String name,
    ...
    if (sd == null) {
      synchronized (ServiceDispatcher.class) {
        if (Debug.verboseOn())
          Debug.logVerbose("[ServiceDispatcher.getInstance] : No instance found (" + dispatcherKey
              + ").", module);
        sd = dispatchers.get(dispatcherKey);
        if (sd == null) {
  ...
  public void register(String name, DispatchContext context) {
    if (Debug.verboseOn())
      Debug.logVerbose("Registered dispatcher: " + context.getName(), module);
  ...
  public Map<String, Object> runSync(String localName,
                                     ...
                                     Map<String, ? extends Object> context) throws ServiceAuthException,
   ...
   * @throws GenericServiceException
   */
  public void runSyncIgnore(String localName,
                            ModelService service,
                            Map<String, ? extends Object> context) throws ServiceAuthException,
                                                                  ServiceValidationException,
                                                                  GenericServiceException {
  ...
  public Map<String, Object> runSync(String localName,
                                     ...
                                     boolean validateOut) throws ServiceAuthException,
                                                         ServiceValidationException,
                                                         GenericServiceException {
    ...
    if (Debug.verboseOn()) {
      Debug.logVerbose("[ServiceDispatcher.runSync] : invoking service " + modelService.name + " ["
          + modelService.location + "/" + modelService.invoke + "] (" + modelService.engineName
          + ")", module);
    }
    ...
    // get eventMap once for all calls for speed, don't do event calls if it is
    // null
    ...
    try {
      ...
      // modelService.requireNewTransaction + ", modelService.useTransaction=" +
      ...
      if (modelService.useTransaction) {
        if (TransactionUtil.isTransactionInPlace()) {
          // if a new transaction is needed, do it here; if not do nothing, just
          // use current tx
          if (modelService.requireNewTransaction) {
            parentTransaction = TransactionUtil.suspend();
            if (TransactionUtil.isTransactionInPlace()) {
              throw new GenericTransactionException("In service " + modelService.name
      ...
      try {
        ...
        do {
          ...
          // NOTE: general pattern here is to do everything up to the main
          // service call, and retry it all if
          // needed because those will be part of the same transaction and have
          // been rolled back
          // TODO: if there is an ECA called async or in a new transaction it
          // won't get rolled back
          // but will be called again, which means the service may complete
          // multiple times! that would be for
          // pre-invoke and earlier events only of course

          // setup global transaction ECA listeners to execute later
          if (eventMap != null)
            ServiceEcaUtil.evalRules(modelService.name,
                                     eventMap,
                                     "global-rollback",
                                     ctx,
                                     context,
          ...
          // "] pre-auth ECA, before auth; isFailure=" + isFailure +
          // ", isError=" + isError, module);
          ...
          if (modelService.auth && userLogin == null) {
            throw new ServiceAuthException("User authorization is required for this service: "
                + modelService.name + modelService.debugInfo());
          }

          // now that we have authed, if there is a userLogin, set the EE
          // userIdentifier
          ...
          // Debug.logInfo("After [" + modelService.name +
          // "] pre-in-validate ECA, before in-validate; isFailure=" + isFailure
          // + ", isError=" + isError, module);
          ...
          if (modelService.validate && !isError && !isFailure) {
            try {
              ...
              Debug.logError(e, "Incoming context (in runSync : " + modelService.name
          ...
          // Debug.logInfo("After [" + modelService.name +
          // "] pre-invoke ECA, before invoke; isFailure=" + isFailure +
          // ", isError=" + isError, module);
          ...
          if (!isError && !isFailure) {
            ...
            if (serviceDebugMode) {
              ...
              modelService.invoker.sendCallbacks(engine,
                                                 context,
                                                 invokeResult,
                                                 null,
            ...
            if (invokeResult != null) {
              ...
              Debug.logWarning("Service (in runSync : " + modelService.name
                  + ") returns null result", module);
            }
          }
          ...
          // Debug.logInfo("After [" + modelService.name +
          // "] invoke; isFailure=" + isFailure + ", isError=" + isError,
          // module);

          if (beganTrans) {
            // crazy stuff here: see if there was a deadlock or other such error
            // and if so retry... which we can ONLY do if we own the
            // transaction!

            String errMsg = ServiceUtil.getErrorMessage(result);

            // look for the string DEADLOCK in an upper-cased error message;
            // tested on: Derby, MySQL
            // - Derby 10.2.2.0 deadlock string:
            // "A lock could not be obtained due to a deadlock"
            // - MySQL ? deadlock string:
            // "Deadlock found when trying to get lock; try restarting transaction"
            // - Postgres ? deadlock string: TODO
            // - Other ? deadlock string: TODO
            // TODO need testing in other databases because they all return
            // different error messages for this!

            // NOTE DEJ20070908 are there other things we need to check? I don't
            // think so because these will
            // be Entity Engine errors that will be caught and come back in an
            // error message... IFF the
            // service is written to not ignore it of course!
            if (errMsg != null && errMsg.toUpperCase().indexOf("DEADLOCK") >= 0) {
              // it's a deadlock! retry...
              String retryMsg = "RETRYING SERVICE [" + modelService.name
                  + "]: Deadlock error found in message [" + errMsg + "]; retry ["
                  + (LOCK_RETRIES - lockRetriesRemaining) + "] of [" + LOCK_RETRIES + "]";

              // make sure the old transaction is rolled back, and then start a
              // new one

              // if there is an exception in these things, let the big overall
              // thing handle it
              ...
              if (!beganTrans) {
                // just log and let things roll through, will be considered an
                // error and ECAs, etc will run according to that
                Debug.logError("After rollback attempt for lock retry did not begin a new transaction!",
                               module);
              } else {
                // deadlocks can be resolved by retring immediately as
                // conflicting operations in the other thread will have cleared
                needsLockRetry = true;
              ...
              // look for lock wait timeout error, retry in a different way by
              // running after the parent transaction finishes, ie attach to
              // parent tx
              // - Derby 10.2.2.0 lock wait timeout string:
              // "A lock could not be obtained within the time requested"
              // - MySQL ? lock wait timeout string:
              // "Lock wait timeout exceeded; try restarting transaction"
              if (errMsg != null
                  && (errMsg.indexOf("A lock could not be obtained within the time requested") >= 0 || errMsg.indexOf("Lock wait timeout exceeded") >= 0)) {
        ...
        // create a new context with the results to pass to ECA services;
        // necessary because caller may reuse this context
        ecaContext = FastMap.newInstance();
        ecaContext.putAll(context);
        // copy all results: don't worry parameters that aren't allowed won't be
        // passed to the ECA services
        ...
        if (modelService.validate && validateOut) {
          ...
          try {
            ...
            throw new GenericServiceException("Outgoing result (in runSync : " + modelService.name
        ...
        // global-commit-post-run ECA, like global-commit but gets the context
        // after the service is run
        if (eventMap != null)
          ServiceEcaUtil.evalRules(modelService.name,
                                   eventMap,
        ...
        if (isFailure) {
          Debug.logWarning("Service Failure [" + modelService.name + "]: "
        ...
        if (t instanceof ServiceAuthException) {
          ...
          throw new GenericServiceException("Service [" + modelService.name + "] Failed"
              + modelService.debugInfo(), t);
        }
      } finally {
        // if there was an error, rollback transaction, otherwise commit
        if (isError) {
          String errMsg = "Error in Service [" + modelService.name + "]: "
              + ServiceUtil.getErrorMessage(result);
          ...
          try {
            ...
            String errMsg = "Could not commit transaction for service [" + modelService.name
                + "] call";
        ...
        // call notifications -- event is determined from the result (success,
        // error, fail)
    ...
    if (Debug.timingOn() && timeToRun > 50) {
      // Sanity check - some service results can be multiple MB in size. Limit
      // message size to 10K.
      String resultStr = result.toString();
      if (resultStr.length() > 10240) {
        resultStr = resultStr.substring(0, 10226) + "...[truncated]";
      }
      Debug.logTiming("Sync service [" + localName + "/" + modelService.name + "] finished in ["
          + timeToRun + "] milliseconds with response [" + resultStr + "]", module);
      ...
      Debug.logInfo("Sync service [" + localName + "/" + modelService.name + "] finished in ["
          + timeToRun + "] milliseconds with response [" + resultStr + "]", module);
    }
  ...
  /**
   * Run the service asynchronously, passing an instance of GenericRequester
   * that will receive the result.
   * 
   * @param localName Name of the context to use.
   * @param service Service model object.
   * @param params Map of name, value pairs composing the parameters.
   * @param requester Object implementing GenericRequester interface which will
   *          receive the result.
   ...
   * @throws GenericServiceException
   */
  public void runAsync(String localName,
                       ModelService service,
                       Map<String, ? extends Object> params,
                       GenericRequester requester,
                       boolean persist) throws ServiceAuthException,
                                       ServiceValidationException,
                                       GenericServiceException {
    if (Debug.timingOn()) {
    ...
    if (Debug.verboseOn()) {
      Debug.logVerbose("[ServiceDispatcher.runAsync] : preparing service " + service.name + " ["
          + service.location + "/" + service.invoke + "] (" + service.engineName + ")", module);
    }
    ...
    try {
      ...
      try {
        // get eventMap once for all calls for speed, don't do event calls if it
        // is null
        Map<String, List<ServiceEcaRule>> eventMap = ServiceEcaUtil.getServiceEventMap(service.name);

        // pre-auth ECA
        if (eventMap != null)
          ServiceEcaUtil.evalRules(service.name,
                                   eventMap,
                                   "auth",
                                   ctx,
                                   context,
        ...
        if (service.auth && userLogin == null) {
          throw new ServiceAuthException("User authorization is required for this service: "
              + service.name + service.debugInfo());
        }
        ...
        if (service.validate && !isError && !isFailure) {
          try {
            ...
            Debug.logError(e, "Incoming service context (in runAsync: " + service.name
                + ") does not match expected requirements", module);
            throw e;
          }
        }
        ...
        if (Debug.timingOn()) {
          UtilTimer.closeTimer(localName + " / " + service.name,
        ...
        if (t instanceof ServiceAuthException) {
          ...
          throw new GenericServiceException("Service [" + service.name + "] Failed"
              + service.debugInfo(), t);
        }
      } finally {
        // always try to commit the transaction since we don't know in this case
        // if its was an error or not
        try {
          TransactionUtil.commit(beganTrans);
        } catch (GenericTransactionException e) {
      ...
      throw new GenericServiceException("Problems with the transaction: " + se.getMessage()
      ...
      if (parentTransaction != null) {
        try {
          ...
          throw new GenericServiceException("Resume transaction exception: " + ise.getMessage()
              + "; See logs for more detail");
        }
      }
    }
  ...
  private Map<String, Object> checkAuth(String localName,
                                        ...
                                        ModelService origService) throws ServiceAuthException,
                                                                 GenericServiceException {
    ...
    if (UtilValidate.isNotEmpty(context.get("login.username"))) {
      // check for a username/password, if there log the user in and make the
      // userLogin object
      ...
      // if a userLogin object is there, make sure the given username/password
      // exists in our local database
      GenericValue userLogin = (GenericValue) context.get("userLogin");

      if (userLogin != null) {
        // Because of encrypted passwords we can't just pass in the encrypted
        // version of the password from the data, so we'll do something
        // different and not run the userLogin service...

        // The old way: GenericValue newUserLogin = getLoginObject(service,
        // localName, userLogin.getString("userLoginId"),
        // userLogin.getString("currentPassword"), (Locale)
        // context.get("locale"));
        GenericValue newUserLogin = null;
        try {
          newUserLogin = this.getDelegator().findByPrimaryKeyCache("UserLogin",
                                                                   "userLoginId",
                                                                   userLogin.get("userLoginId"));
        } catch (GenericEntityException e) {
          Debug.logError(e,
                         "Error looking up service authentication UserLogin: " + e.toString(),
                         module);
        ...
        if (newUserLogin == null) {
          ...
          // we'll have to remove it from the incoming context which will cause
          // an auth error later if auth is required
          Debug.logInfo("Service auth failed for userLoginId [" + userLogin.get("userLoginId")
              + "] because UserLogin record not found.", module);
          context.remove("userLogin");
        } else if (newUserLogin.getString("currentPassword") != null
            && !newUserLogin.getString("currentPassword")
                            .equals(userLogin.getString("currentPassword"))) {
          // passwords didn't match, remove the userLogin for failed auth
          Debug.logInfo("Service auth failed for userLoginId ["
                            + userLogin.get("userLoginId")
                            + "] because UserLogin record currentPassword fields did not match; note that the UserLogin object passed into a service may need to have the currentPassword encrypted.",
                        module);
          context.remove("userLogin");
        }
      }
    ...
    if (UtilValidate.isNotEmpty(origService.permissionServiceName)) {
      ...
      if (!origService.evalPermissions(dctx, context)) {
        throw new ServiceAuthException("You do not have permission to invoke the service ["
            + origService.name + "]");
      }
    }
  ...
  private GenericValue getLoginObject(String service,
                                      String localName,
                                      String username,
                                      String password,
                                      Locale locale) throws GenericServiceException {
    Map<String, Object> context = FastMap.newInstance();
    context.putAll(UtilMisc.toMap("login.username",
                                  username,
                                  "login.password",
                                  password,
                                  "isServiceAuth",
  ...
  /**
   * Enables/Disables the Job Manager/Scheduler globally (this will not effect
   * any dispatchers already running)
   * 
   * @param enable
   */
  ...
  /**
   * Enables/Disables the JMS listeners globally (this will not effect any
   * dispatchers already running)
   * 
   * @param enable
   */
  ...
  /**
   * Enables/Disables the startup services globally (this will not effect any
   * dispatchers already running)
   * 
   * @param enable
   */
