...
/**
 * A LocalDispatcher is associated with an application. 
 * Applications never talk directly to the ServiceDispatcher. 
 * The LocalDispatcher contains an API for invoking services, 
 * which are routed through the ServiceDispather. However, 
 * applications may be running in different threads than 
 * the actual ServiceDispatcher, so it is left to the LocalDispatcher 
 * to keep a DispatchContext which among other things keeps 
 * a reference to the applications classloader.
 */
