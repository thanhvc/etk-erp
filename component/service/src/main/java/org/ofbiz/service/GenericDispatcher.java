 ...
 * Service Engine where it is then invoked. There is exactly one ServiceDispatcher 
 * for each Entity Delegator. If there are multiple delegators in an application 
 * there will also be multiple dispatchers. The ServiceDispatcher is accessed via a LocalDispatcher. 
 * There can be many LocalDispatchers associated with a ServiceDispatcher. 
 * Each LocalDispatcher is uniquely named and contains its own list of service definitions. 
 * When creating an instance of a LocalDispatcher, a DispatchContext is also created and 
 * passed to the ServiceEngine.
 * 
 */
