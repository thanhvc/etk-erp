...
public class ContainerLoader implements StartupLoader {
   ...
   * <code>null</code> if the specified container was not loaded. If more than
   * one instance of the container was loaded, then the last instance that was
   * loaded is returned. The returned <code>Container</code> will be
   * initialized, but there is no guarantee that it will be in the running
   * state.
   * 
  ...
  public synchronized void load(Config config, String args[]) throws StartupException {
    ...
    for (ContainerConfig.Container containerCfg : containers) {
      ...
      // This is only used in case of OFBiz running in Geronimo or WASCE. It
      // allows to use the RMIDispatcher
      // FIXME @ThanhVC: Need to config RMIDispatcher and also to set Ofbiz
      // ClassLoader to Tomcat Embedded Context set
      // via tmpContainer.start();
      if (containerCfg.name.equals("rmi-dispatcher") && configFile.equals("limited-containers.xml")) {
        try {
          ...
          if (!useCtx.equalsIgnoreCase("true")) {
            ...
            if (System.getSecurityManager() == null) { // needed by WASCE with a
                                                       // client.policy file.
