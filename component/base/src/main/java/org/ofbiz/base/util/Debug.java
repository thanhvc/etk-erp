...
public final class Debug {
  ...
  public static final String noModuleModule = "NoModule"; // set to null for
                                                          // previous behavior
  ...
  static {
    ...
    // initialize Log4J
    if (!UtilProperties.propertyValueEqualsIgnoreCase("config/debug.properties", "disable.log4j.config", "true")) {
      org.apache.log4j.xml.DOMConfigurator.configure(UtilURL.fromResource("config/log4j.xml"));
    }

    // initialize levelOnCache
    for (int i = 0; i < 9; i++) {
      levelOnCache[i] = (i == Debug.ALWAYS || UtilProperties.propertyValueEqualsIgnoreCase("config/debug.properties", levelProps[i], "true"));
    }
    ...
    packException = UtilProperties.propertyValueEqualsIgnoreCase("config/debug.properties", "pack.exception", "true");
  }
  ...
  /**
   * Gets an Integer representing the level number from a String representing
   * the level name; will return null if not found
   */
  ...
  /**
   * Gets an int representing the level number from a String representing the
   * level name; if level not found defaults to Debug.INFO
   */
  ...
  public static boolean isOn(int level) {
    if (useLevelOnCache) {
      ...
      return (level == Debug.ALWAYS || UtilProperties.propertyValueEqualsIgnoreCase("config/debug", levelProps[level], "true"));
    }
  }
