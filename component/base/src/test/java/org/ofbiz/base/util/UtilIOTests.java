...
public class UtilIOTests extends GenericTestCaseBase {
  private static final byte[] trademarkBytes = new byte[] { (byte) 0xE2, (byte) 0x84, (byte) 0xA2 };
  ...
  private static void readStringTest_1(String label, String wanted, byte[] toRead) throws IOException {
    ...
    assertEquals("readString bytes offset/length default:" + label,
                 wanted,
                 UtilIO.readString(toRead, 0, toRead.length));
    assertEquals("readString bytes offset/length UTF-8:" + label,
                 wanted,
                 UtilIO.readString(toRead, 0, toRead.length, "UTF-8"));
    assertEquals("readString bytes offset/length UTF8:" + label,
                 wanted,
                 UtilIO.readString(toRead, 0, toRead.length, UtilIO.UTF8));
    assertEquals("readString stream default:" + label,
                 wanted,
                 UtilIO.readString(new ByteArrayInputStream(toRead)));
    assertEquals("readString stream UTF-8:" + label,
                 wanted,
                 UtilIO.readString(new ByteArrayInputStream(toRead), "UTF-8"));
    assertEquals("readString stream UTF8:" + label,
                 wanted,
                 UtilIO.readString(new ByteArrayInputStream(toRead), UtilIO.UTF8));
  }
  ...
  protected void checkBasicReadObject(Object value, String text) throws Exception {
    ...
    assertEquals("read bytes " + value.getClass().getName(),
                 value,
                 UtilIO.readObject(new ByteArrayInputStream(bytes)));
    assertEquals("read chars " + value.getClass().getName(),
                 value,
                 UtilIO.readObject(text.toCharArray()));
    assertEquals("read chars offset " + value.getClass().getName(),
                 value,
  ...
  protected void checkBasicReadWriteObject(Object value, String text) throws Exception {
    ...
    assertEquals("write stream " + value.getClass().getName(), text, new String(baos.toByteArray(),
                                                                                "UTF-8"));
  ...
  public void testReadWriteObject() throws Exception {
    ...
    checkBasicReadWriteObject(InetAddress.getByAddress("localhost", new byte[] { 127, 0, 0, 1 }),
