...
public class EntityDataLoadContainer implements Container {
  ...
  public void init(String[] args, String configFile) throws ContainerException {
    ...
    /*
     * install arguments: readers (none, all, seed, demo, ext, etc - configured
     * in entityengine.xml and associated via ofbiz-component.xml) timeout
     * (transaction timeout default 7200) delegator (overrides the delegator
     * name configured for the container) group (overrides the entity group name
     * configured for the container) dir (imports all XML files in a directory)
     * file (import a specific XML file) Example: $ java -jar ofbiz.jar -install
     * -readers=seed,demo,ext -timeout=7200 -delegator=default -group=org.ofbiz
    ...
    if (args != null) {
      for (String argument : args) {
        ...
        if ("readers".equalsIgnoreCase(argumentName)) {
          ...
          Debug.log("--------------------------------------", module);
          Debug.log("java -jar start.jar -install [options]", module);
  ...
  public boolean start() throws ContainerException {
    //ThanhVC install-containers.xml
    /*
    ...
    if (helperInfo == null) {
      throw new ContainerException("Unable to locate the datasource helper for the group ["
          + groupNameToUse + "]");
    }
          ...
          for (File file : fileArray) {
            if (file.getName().toLowerCase().endsWith(".xml")) {
              try {
                ...
                Debug.logError(e,
                               "Unable to load file (" + file.getName() + "); not a valid URL.",
                               module);
              }
            }
          }
    ...
    Debug.logImportant("=-=-=-=-=-=-= Finished the data load with " + totalRowsChanged
        + " rows changed.", module);
