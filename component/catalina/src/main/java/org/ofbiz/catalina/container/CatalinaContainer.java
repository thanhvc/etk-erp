...
public class CatalinaContainer implements Container {
  ...
  public static final String                                CATALINA_HOSTS_HOME    = System.getProperty("ofbiz.home") + "/component/catalina/hosts";
  ...
  public boolean start() throws ContainerException {
    ...
    for (Connector con : embedded.findConnectors()) {
      ...
      if (ph instanceof Http11Protocol) {
        ...
        Debug.logInfo("Connector " + hph.getProtocols() + " @ " + hph.getPort() + " - " + (hph.getSecure() ? "secure" : "not-secure") + " [" + con.getProtocolHandlerClassName() + "] started.", module);
      } else {
  ...
  private StandardContext configureContext(Engine engine, Host host, ComponentConfig.WebappInfo appInfo) throws ContainerException {
    ...
    //FIXME THANHVC - Set loader for Tomcat engine
      ...
      for (int i = webResourceInfos.size(); i > 0; i--) {
        ...
        if (!keys.removeAll(loadedMounts)) {
          ...
          if (appInfo.location != null) {
            // TODO Here inportant to create Context for Tomcat
            futures.add(executor.submit(createContext(appInfo)));
          }
          loadedMounts.addAll(keys);
        } else {
          appInfo.appBarDisplay = false; // disable app bar display on overrided
                                         // apps
