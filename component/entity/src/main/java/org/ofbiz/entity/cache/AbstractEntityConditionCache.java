...
public abstract class AbstractEntityConditionCache<K, V> extends AbstractCache<EntityCondition, Map<K, V>> {
  ...
  protected V get(String entityName, EntityCondition condition, K key) {
    ...
    if (conditionCache == null)
      return null;
    // the following line was synchronized, but for pretty good safety and
    // better performance, only syncrhnizing the put; synchronized
    // (conditionCache) {
  ...
  protected <T1 extends Map<String, Object>, T2 extends Map<String, Object>> void storeHook(String entityName, boolean isPK, List<T1> oldValues, List<T2> newValues) {
    ...
    if (UtilValidate.isEmpty(newValues) || newValues.get(0) == null) {
      ...
      // entityName + "], got entity cache with name: " + (entityCache == null ?
    ...
    for (EntityCondition condition : entityCache.getCacheLineKeys()) {
      ...
      if (condition == null) {
        ...
        while (oldValueIter.hasNext() && !shouldRemove) {
          ...
          if (condition.mapMatches(getDelegator(), oldValue)) {
            oldMatched = true;
            // Debug.logInfo("In storeHook, oldMatched for entityName [" +
            // entityName + "]; shouldRemove is false", module);
            if (newValues != null) {
              Iterator<T2> newValueIter = newValues.iterator();
              while (newValueIter.hasNext() && !shouldRemove) {
                T2 newValue = newValueIter.next();
                shouldRemove |= isNull(newValue) || condition.mapMatches(getDelegator(), newValue);
                // Debug.logInfo("In storeHook, for entityName [" + entityName +
                // "] shouldRemove is now " + shouldRemove, module);
              }
            } else {
              shouldRemove = true;
            }
        ...
        if (!oldMatched && isPK) {
          // Debug.logInfo("In storeHook, for entityName [" + entityName +
          // "] oldMatched is false and isPK is true, so setting shouldRemove to true (will remove from cache)",
          // module);
          shouldRemove = true;
        }
      }
      if (shouldRemove) {
        ...
        // doesn't work anymore since this is a copy of the cache keySet, can
        // call remove directly though with a concurrent mod exception:
        // cacheKeyIter.remove();
