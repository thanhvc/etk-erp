...
public class EntityDateFilterCondition extends EntityCondition {
  ...
  public static EntityExpr makeCondition(Timestamp moment, String fromDateName, String thruDateName) {
    return EntityCondition.makeCondition(EntityCondition.makeCondition(EntityCondition.makeCondition(thruDateName, EntityOperator.EQUALS, null), EntityOperator.OR, EntityCondition.makeCondition(thruDateName, EntityOperator.GREATER_THAN, moment)), EntityOperator.AND, EntityCondition.makeCondition(EntityCondition.makeCondition(fromDateName, EntityOperator.EQUALS, null), EntityOperator.OR, EntityCondition.makeCondition(fromDateName, EntityOperator.LESS_THAN_EQUAL_TO, moment)));
  }

  /**
   * Creates an EntityCondition representing a date range filter query to be
   * used against entities that themselves represent a date range. When used the
   * resulting entities will meet at least one of the following criteria: -
   * fromDate is equal to or after rangeStart but before rangeEnd - thruDate is
   * equal to or after rangeStart but before rangeEnd - fromDate is null and
   * thruDate is equal to or after rangeStart - thruDate is null and fromDate is
   * before rangeEnd - fromDate is null and thruDate is null
   * 
