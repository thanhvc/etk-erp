...
public class ModelUtil {
  ...
  /**
   * Converts a database name to a Java class name. The naming conventions used
   * to allow for this are as follows: a database name (table or column) is in
   * all capital letters, and the words are separated by an underscore (for
   * example: NEAT_ENTITY_NAME or RANDOM_FIELD_NAME); a Java name (ejb or field)
   * is in all lower case letters, except the letter at the beginning of each
   * word (for example: NeatEntityName or RandomFieldName). The convention of
   * using a capital letter at the beginning of a class name in Java, or a
   * lower-case letter for the beginning of a variable name in Java is also used
   ...
   * used to allow for this are as follows: a database name (table or column) is
   * in all capital letters, and the words are separated by an underscore (for
   * example: NEAT_ENTITY_NAME or RANDOM_FIELD_NAME); a Java name (ejb or field)
   * is in all lower case letters, except the letter at the beginning of each
   * word (for example: NeatEntityName or RandomFieldName). The convention of
   * using a capital letter at the beginning of a class name in Java, or a
   ...
   * each _ separated segment, go until it is less than or equal to the desired
   * length
   * 
   * @param dbName
   * @param desiredLength
  ...
  public static String shortenDbName(String dbName, int desiredLength) {
    ...
    if (dbBuf.length() > desiredLength) {
      ...
      for (int i = dbBuf.length() - 1; i > 0; i--) {
        // don't remove vowels that are at the beginning of the string (taken
        // care of by the i > 0) or right after an underscore
    ...
    while (dbBuf.length() > desiredLength) {
      ...
      while (usIndex > 0 && dbBuf.length() > desiredLength) {
        // if this is the first word in the group, don't pull letters off unless
        // it is 4 letters or more
        int prevUsIndex = dbBuf.lastIndexOf("_", usIndex - 1);
        if (prevUsIndex < 0 && usIndex < 4) {
          break;
        }

        // don't remove characters to reduce the size two less than three
        ...
        // delete the second to last character instead of the last, better
    ...
    while (dbBuf.length() > desiredLength) {
      ...
      // don't remove the first segment, just remove the second over and over
      // until we are short enough
  ...
  /**
   * Converts a package name to a path by replacing all '.' characters with the
   * File.separatorChar character. Is therefore platform independent.
   * 
