    ...
    if (datasourceInfo == null) {
      ...
      try {
        if (datasourceInfo.resultFetchSize > 1)
          con = CursorConnection.newCursorConnection(con,
                                                     datasourceInfo.cursorName,
                                                     datasourceInfo.resultFetchSize);
      } catch (Exception ex) {
        Debug.logWarning(ex,
                         "Error creating the cursor connection proxy "
                             + helperInfo.getHelperBaseName() + " datasource.",
                         module);
      }
    }
    return con;
  ...
  public static Connection getCursorConnection(String helperName, Connection con) {
    ...
    if (datasourceInfo == null) {
      ...
      try {
        ...
        Debug.logWarning(ex, "Error creating the cursor connection proxy " + helperName
            + " datasource.", module);
      }
    }
    return con;
