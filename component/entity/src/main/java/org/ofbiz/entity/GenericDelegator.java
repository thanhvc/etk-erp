...
public class GenericDelegator implements Delegator {
  ...
  /**
   * keeps a list of field key sets used in the by and cache, a Set (of Sets of
   ...
   * (usually the userLoginId, though technically the Entity Engine doesn't know
   * anything about the UserLogin entity)
   */
  ...
  protected GenericDelegator(String delegatorFullName) throws GenericEntityException {
    ...
    // before continuing, if there is a tenantId use the base delegator to see
    // if it is valid
    ...
    // NOTE: doing some things before the ECAs and such to make sure it is in
    // place just in case it is used in a service engine startup thing or
    // something
  ...
  public synchronized void initEntityEcaHandler() {
    // Nothing to do if already assigned: the class loader has already been
    // called, the class instantiated and casted to EntityEcaHandler
    if (this.entityEcaHandler != null) {
      return;
    }
    // If useEntityEca is false do nothing: the entityEcaHandler member field
    // with a null value would cause its code to do nothing
    if (getDelegatorInfo().useEntityEca) {
      ...
      try {
        ...
        enableJMS(!getDelegatorInfo().useDistributedCacheClear); // To avoid
                                                                 // duplicated
                                                                 // JMS
                                                                 // listeners
                                                                 // (OFBIZ-4296)
  ...
  public GenericHelperInfo getGroupHelperInfo(String entityGroupName) {
    ...
    if (UtilValidate.isNotEmpty(this.delegatorTenantId) && !"org.ofbiz.tenant".equals(entityGroupName)) {
      ...
      // tenantId
      try {
        // NOTE: instead of caching the GenericHelpInfo object do a cached query
        // here and create a new object each time, will avoid issues when the
        // database data changes during run time
        // NOTE: always use the base delegator for this to avoid problems when
        ...
        if (tenantDataSource != null) {
          ...
          /*
           * don't log this, happens too many times: if (Debug.warningOn()) {
           * Debug.logWarning(
           * "Could not find TenantDataSource information for tenantId=[" +
           * this.delegatorTenantId + "] and entityGroupName=[" +
           * entityGroupName + "] in delegator [" + this.delegatorFullName +
           * "]; will be defaulting to settings for the base delegator name [" +
           * this.delegatorBaseName + "]", module); }
           */
        }
      } catch (GenericEntityException e) {
        // don't complain about this too much, just log the error if there is
        // one
  ...
  /*
   * (non-Javadoc)
   * @see org.ofbiz.entity.Delegator#getEntityHelperName(org.ofbiz.entity.model.
   * ModelEntity)
   */
  ...
  /*
   * (non-Javadoc)
   * @see org.ofbiz.entity.Delegator#getEntityFieldType(org.ofbiz.entity.model.
  ...
  public GenericValue createSetNextSeqId(GenericValue value) throws GenericEntityException {
    ...
    value.setDelegator(this);
    // this will throw an IllegalArgumentException if the entity for the value
    // does not have one pk field, or if it already has a value set for the one
    // pk field
    value.setNextSeqId();
    ...
    try {
      ...
      try {
        ...
        // see if this was caused by an existing record before resetting the
        // sequencer and trying again
        ...
        try {
          ...
          // it is important to not let this get thrown because it will mask the
          // original exception
        }
        if (existingValue == null) {
          ...
          // found an existing value... was probably a duplicate key, so clean
  ...
  public int removeByPrimaryKey(GenericPK primaryKey, boolean doCacheClear) throws GenericEntityException {
    ...
    try {
      ...
      // if audit log on for any fields, save old value before removing so it's
      // still there
  ...
  public int removeValue(GenericValue value, boolean doCacheClear) throws GenericEntityException {
    // NOTE: this does not call the GenericDelegator.removeByPrimaryKey method
    // because it has more information to pass to the ECA rule hander
    boolean beganTransaction = false;
    try {
      if (alwaysUseTransaction) {
        beganTransaction = TransactionUtil.begin();
  ...
  public int store(GenericValue value, boolean doCacheClear) throws GenericEntityException {
    ...
    try {
      ...
      // if audit log on for any fields, save old value before the update so we
      // still have both
      if (value.getModelEntity().getHasFieldWithAuditLog()) {
  ...
  public int storeAll(List<GenericValue> values, boolean doCacheClear, boolean createDummyFks) throws GenericEntityException {
    ...
    try {
      ...
      for (GenericValue value : values) {
        ...
        // exists?
        // NOTE: don't use findByPrimaryKey because we don't want to the ECA
        // events to fire and such
        ...
        if (existing == null) {
          ...
          // don't send fields that are the same, and if no fields have changed,
          // update nothing
          ModelEntity modelEntity = value.getModelEntity();
  ...
  /*
   * (non-Javadoc)
   * @see org.ofbiz.entity.Delegator#findByPrimaryKey(java.lang.String,
   * java.util.Map)
   */
  ...
  /*
   * (non-Javadoc)
   * @see org.ofbiz.entity.Delegator#findByPrimaryKeyCache(java.lang.String,
   * java.lang.Object)
   */
   ...
   * (non-Javadoc)
   * @see
   * org.ofbiz.entity.Delegator#findByPrimaryKeyPartial(org.ofbiz.entity.GenericPK
   * , java.util.Set)
   */
  ...
  /*
   * (non-Javadoc)
   * @see org.ofbiz.entity.Delegator#findByAndCache(java.lang.String,
   * java.util.Map)
   */
  ...
  public EntityListIterator find(String entityName, EntityCondition whereEntityCondition, EntityCondition havingEntityCondition, Set<String> fieldsToSelect, List<String> orderBy, EntityFindOptions findOptions) throws GenericEntityException {
    ...
    // if there is no transaction throw an exception, we don't want to create a
    // transaction here since closing it would mess up the ELI
    if (!TransactionUtil.isTransactionInPlace()) {
      ...
      // throwing an exception is a little harsh for now, just display a really
      // big error message since we want to get all of these fixed...
   ...
   * org.ofbiz.entity.condition.EntityCondition, java.util.Set, java.util.List,
   * org.ofbiz.entity.util.EntityFindOptions, boolean)
   */
  ...
  /*
   * (non-Javadoc)
   * @see org.ofbiz.entity.Delegator#findCountByCondition(java.lang.String,
   * org.ofbiz.entity.condition.EntityCondition,
   * org.ofbiz.entity.condition.EntityCondition,
   * org.ofbiz.entity.util.EntityFindOptions)
   */
  ...
  /*
   * (non-Javadoc)
   * @see org.ofbiz.entity.Delegator#getRelated(java.lang.String, java.util.Map,
   * java.util.List, org.ofbiz.entity.GenericValue)
   */
  ...
  /*
   * (non-Javadoc)
   * @see org.ofbiz.entity.Delegator#clearCacheLineByCondition(java.lang.String,
   * org.ofbiz.entity.condition.EntityCondition)
   */
  ...
  public void clearCacheLine(GenericValue value, boolean distribute) {
    // TODO: make this a bit more intelligent by passing in the operation being
    // done (create, update, remove) so we can not do unnecessary cache
    // clears...
    // for instance:
    // on create don't clear by primary cache (and won't clear original values
   ...
   * (non-Javadoc)
   * @see
   * org.ofbiz.entity.Delegator#setDistributedCacheClear(org.ofbiz.entity.util
  ...
  public String getNextSeqId(String seqName, long staggerMax) {
    ...
    if (nextSeqLong == null) {
      // NOTE: the getNextSeqIdLong method SHOULD throw a runtime exception when
      // no sequence value is found, which means we should never see it get here
  ...
  public void setNextSubSeqId(GenericValue value, String seqFieldName, int numericPadding, int incrementBy) {
    if (value != null && UtilValidate.isEmpty(value.getString(seqFieldName))) {
      ...
      try {
        ...
        // get values in whatever order, we will go through all of them to find
        // the highest value
        ...
        // ", and the seqFieldName: " + seqFieldName + ", and the results are: "
        // + allValues, module);
  ...
  public GenericDelegator cloneDelegator(String delegatorFullName) {
    ...
    // set the delegatorBaseName to be the same so that configuration settings
    // all work the same as the current
    // delegator, allowing the new delegatorFullName to not match a delegator
    // name in the entityengine.xml file
  ...
  public void initDistributedCacheClear() {
    // Nothing to do if already assigned: the class loader has already been
    // called, the class instantiated and casted to DistributedCacheClear
    if (this.distributedCacheClear != null) {
      return;
    }

    // If useDistributedCacheClear is false do nothing: the
    // distributedCacheClear member field with a null value would cause dcc code
    // to do nothing
