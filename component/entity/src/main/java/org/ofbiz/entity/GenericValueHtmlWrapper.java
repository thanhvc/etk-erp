...
public class GenericValueHtmlWrapper extends GenericValue {
  ...
  /*
   * NOTE: this is NOT used because there are certain FTL files that call
   * services and things, and this messes those up, so only overriding the
   * Map.get(Object) method to get use of this as a Map Override the basic get
   * method, which all other get methods call so we only need to do this one
   * (though most important for the Map.get(Object) and the getString() methods
   * public Object get(String name) { Object value = super.get(name); if (value
   * instanceof String) { return StringUtil.htmlEncoder.encode((String) value);
   * } else { return value; } }
   */
