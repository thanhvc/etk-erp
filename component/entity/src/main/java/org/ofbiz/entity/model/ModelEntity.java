...
public class ModelEntity extends ModelInfo implements Comparable<ModelEntity>, Serializable {
  ...
  /**
   * An indicator to specify if this entity is never cached. If true causes the
   * delegator to not clear caches on write and to not get from cache on read
   * showing a warning messages to that effect
   */
  ...
  public ModelEntity(String tableName, Map<String, DatabaseUtil.ColumnCheckInfo> colMap, ModelFieldTypeReader modelFieldTypeReader, boolean isCaseSensitive) {
    // if there is a dot in the name, remove it and everything before it, should
    // be the schema name
  ...
  public void addExtendEntity(ModelReader reader, Element extendEntityElement) {
    ...
    for (Element fieldElement : UtilXml.childElementList(extendEntityElement, "field")) {
      ...
      if (field != null) {
        ...
        if (existingField != null) {
          ...
          // TODO: only overrides of type, colName and description are currently
          // supported
          ...
          field.setModelEntity(this);
          this.fields.add(field);
          // this will always be true for now as extend-entity fielsd are always
          // nonpks
          if (!field.isPk)
            this.nopks.add(field);
        }
    ...
    // Debug.log("Extended entity - " +
    // extendEntityElement.getAttribute("entity-name") + " new resource name : "
    // + defResourceName, module);
  ...
  public ModelField getField(String fieldName) {
    ...
    if (modelField == null) {
      // sometimes weird things happen and this getField method is called before
      // the fields are all populated, so before moving on just re-create the
      // fieldsMap again real quick...
      // the purpose of the fieldsMap is for speed, but if failures are a little
      // slower, no biggie
      createFieldsMap();
  ...
  public ModelRelation getRelation(String relationName) {
    ...
    for (ModelRelation relation : relations) {
      if (relationName.equals(relation.title + relation.relEntityName))
        return relation;
    }
    return null;
  }
   ...
   * returnString = ""; if (relation.keyMaps.size() < 1) { return ""; } int i =
   * 0; for(; i < relation.keyMaps.size() - 1; i++) { ModelKeyMap keyMap =
   * (ModelKeyMap)relation.keyMaps.get(i); if (keyMap != null) returnString =
   * returnString + "\"" + tableName + "_" + keyMap.relColName + "=\" + " +
   * ModelUtil.lowerFirstChar(relation.mainEntity.entityName) + ".get" +
   * ModelUtil.upperFirstChar(keyMap.fieldName) + "() + \"&\" + "; } ModelKeyMap
   * keyMap = (ModelKeyMap)relation.keyMaps.get(i); returnString = returnString
   * + "\"" + tableName + "_" + keyMap.relColName + "=\" + " +
   * ModelUtil.lowerFirstChar(relation.mainEntity.entityName) + ".get" +
  ...
  public int compareTo(ModelEntity otherModelEntity) {
     ...
     * dependencies if
     * (this.getEntityName().equals(otherModelEntity.getEntityName())) { return
     * 0; } //look through relations for dependencies from this entity to the
     * other Iterator relationsIter = this.getRelationsIterator(); while
     * (relationsIter.hasNext()) { ModelRelation modelRelation = (ModelRelation)
     * relationsIter.next(); if ("one".equals(modelRelation.getType()) &&
     * modelRelation
     * .getRelEntityName().equals(otherModelEntity.getEntityName())) { //this
     * entity is dependent on the other entity, so put that entity earlier in
     * the list return -1; } } //look through relations for dependencies from
     * the other to this entity Iterator otherRelationsIter =
     * otherModelEntity.getRelationsIterator(); while
     * (otherRelationsIter.hasNext()) { ModelRelation modelRelation =
     * (ModelRelation) otherRelationsIter.next(); if
     * ("one".equals(modelRelation.getType()) &&
     * modelRelation.getRelEntityName().equals(this.getEntityName())) { //the
     * other entity is dependent on this entity, so put that entity later in the
     * list return 1; } } return 0;
     */
  ...
  /**
   * Writes entity model information in the Apple EOModelBundle format. For
   * document structure and definition see:
   * http://developer.apple.com/documentation
   * /InternetWeb/Reference/WO_BundleReference/Articles/EOModelBundle.html For
   * examples see the JavaRealEstate.framework and JavaBusinessLogic.framework
   * packages which are in the /Library/Frameworks directory after installing
   * the WebObjects Examples package (get latest version of WebObjects download
   * for this). This is based on examples and documentation from WebObjects 5.4,
   * downloaded 20080221.
   * 
   * @param writer
   * @param entityPrefix
  ...
  public Map<String, Object> createEoModelMap(String entityPrefix, String helperName, Set<String> entityNameIncludeSet, ModelReader entityModelReader) throws GenericEntityException {
    ...
    // for classProperties add field names AND relationship names to get a nice,
    // complete chart
