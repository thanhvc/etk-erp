...
public class SQLProcessor {
   ...
   * 
   * @param helperInfo The datasource helper (see entityengine.xml
   *          &lt;datasource name=".."&gt;)
   */
  ...
  ResultSetMetaData getResultSetMetaData() {
    if (_rsmd == null) {
      // try the ResultSet, if not null, or try the PreparedStatement, also if
      // not null
      try {
        if (_rs != null) {
  ...
  public Connection getConnection() throws GenericDataSourceException, GenericEntityException {
    ...
    /*
     * causes problems w/ postgres ?? if (Debug.verboseOn()) { int isoLevel =
     * -999; try { isoLevel = _connection.getTransactionIsolation(); } catch
     * (SQLException e) { Debug.logError(e,
     * "Problems getting the connection's isolation level", module); } if
     ...
     * Debug.logVerbose("Transaction isolation level set to 'ReadCommited'.",
     * module); } else if (isoLevel == Connection.TRANSACTION_READ_UNCOMMITTED)
     * {
     * Debug.logVerbose("Transaction isolation level set to 'ReadUncommitted'.",
     * module); } else if (isoLevel == Connection.TRANSACTION_REPEATABLE_READ) {
     * Debug.logVerbose("Transaction isolation level set to 'RepeatableRead'.",
     * module); } else if (isoLevel == Connection.TRANSACTION_SERIALIZABLE) {
     * Debug.logVerbose("Transaction isolation level set to 'Serializable'.",
     * module); } }
     */

    // always try to set auto commit to false, but if we can't then later on we
    // won't commit
    try {
  ...
  public ResultSet executeQuery() throws GenericDataSourceException {
    try {
      // if (Debug.verboseOn())
      // Debug.logVerbose("[SQLProcessor.executeQuery] ps=" + _ps.toString(),
      // module);
  ...
  public int executeUpdate() throws GenericDataSourceException {
    try {
      // if (Debug.verboseOn())
      // Debug.logVerbose("[SQLProcessor.executeUpdate] ps=" + _ps.toString(),
      // module);
      // TransactionUtil.printAllThreadsTransactionBeginStacks();
      return _ps.executeUpdate();
    } catch (SQLException sqle) {
      this.checkLockWaitInfo(sqle);
      // don't display this here, may not be critical, allow handling further
      // up... Debug.logError(sqle, "SQLProcessor.executeUpdate() : ERROR : ",
      // module);
      throw new GenericDataSourceException("SQL Exception while executing the following:" + _sql, sqle);
    }
  }
  ...
  public int executeUpdate(String sql) throws GenericDataSourceException {
    ...
    try {
      ...
      // Debug.logError(sqle, "SQLProcessor.executeUpdate(sql) : ERROR : ",
      // module);
      throw new GenericDataSourceException("SQL Exception while executing the following:" + _sql, sqle);
    } finally {
      if (stmt != null) {
  ...
  /**
   * Execute a query based on the SQL string given. For each record of the
   * ResultSet return, execute a callback function
   * 
   * @param sql The SQL string to be executed
  ...
  public void setValue(String field) throws SQLException {
    ...
    // this doesn't seem to work, query not yet parsed: int colType =
    // rsmd.getColumnType(_ind);
    if (field != null) {
      // if (field.length() > 4000) {
      // Clob clb = new Cl
      // doesn't work with Oracle drivers, need the funky work-around:
      // _ps.setCharacterStream(_ind, new StringReader(field), field.length());
  ...
  /**
   * Set the next binding variable of the currently active prepared statement to
   * write the serialized data of 'field' to a BLOB.
   * 
   * @param field
   * @throws SQLException
   */
  ...
  /**
   * Set the next binding variable of the currently active prepared statement to
   * write the serialized data of 'field' to a Blob with the given bytes.
   * 
   * @param bytes
   * @throws SQLException
   */
  ...
  protected void setFetchSize(Statement stmt, int fetchSize) throws SQLException {
    ...
    if (_connection instanceof CursorConnection)
      return;

    // check if the statement was called with a specific fetch size, if not grab
    // the default from the datasource
    if (fetchSize < 0) {
    ...
    // otherwise only set if the size is > -1 (0 is sometimes used to note ALL
    // rows)
    if (fetchSize > -1) {
  ...
  private void checkLockWaitInfo(Exception sqle) {
    ...
    // see if there is a lock wait timeout error, if so try to get and print
    // more info about it
    // the string for Derby is
    // "A lock could not be obtained within the time requested"
    // the string for MySQL is
