 ...
 * the field will be passed to the <code>notifyObservers()</code> method, and
 * through that to the <code>update()</code> method of each
 * <code>Observer</code>.
 */
...
public class GenericEntity extends Observable implements Map<String, Object>, LocalizedMap<Object>, Serializable, Comparable<GenericEntity>, Cloneable, Reusable {
   ...
   * writes are done through synchronized setters) and being able to store null
   * values. Null values are important because with them we can distinguish
  ...
  /**
   * Contains the entityName of this entity, necessary for efficiency when
   * creating EJBs
   */
  protected String entityName = null;

  /**
   * Contains the ModelEntity instance that represents the definition of this
   * entity, not to be serialized
   */
  protected transient ModelEntity modelEntity = null;

  /**
   * Denotes whether or not this entity has been modified, or is known to be out
  ...
  public void setAllFields(Map<? extends Object, ? extends Object> fields, boolean setIfEmpty, String namePrefix, Boolean pks) {
    ...
    while (iter != null && iter.hasNext()) {
      ...
      if (fields.containsKey(sourceFieldName)) {
        ...
        // if (Debug.verboseOn()) Debug.logVerbose("Setting field " +
        // curField.getName() + ": " + field + ", setIfEmpty = " + setIfEmpty,
        // module);
        if (setIfEmpty) {
  ...
  public void setFields(Map<? extends String, ? extends Object> keyValuePairs) {
    if (keyValuePairs == null)
      return;
    // this could be implement with Map.putAll, but we'll leave it like this for
  ...
  public void writeXmlText(PrintWriter writer, String prefix) {
    ...
    // write attributes immediately and if a CDATA element is needed, put those
    // in a Map for now
    ...
    while (modelFields.hasNext()) {
      ...
      if (type != null && type.equals("blob")) {
        ...
        if (valueStr != null) {
          ...
          // check each character, if line-feed or carriage-return is found set
          // needsCdata to true; also look for invalid characters
          for (int i = 0; i < value.length(); i++) {
            char curChar = value.charAt(i);
            /*
             * Some common character for these invalid values, have seen these
             * are mostly from MS Word, but may be part of some standard: 5 =
             * ... 18 = apostrophe 19 = left quotation mark 20 = right quotation
             * mark 22 = – 23 = - 25 = tm
             */
            ...
            default:
              if (curChar < 0x20) {
                // if it is less that 0x20 at this point it is invalid because
                // the only valid values < 0x20 are 0x9, 0xA, 0xD as caught
                // above
                ...
                // Replace each char which is out of the ASCII range with a XML
                // entity
          ...
          if (needsCdata) {
            // use valueStr instead of the escaped value, not needed or wanted
            // in a CDATA block
  ...
  /**
   * Creates a String for the entity, overrides the default toString This method
   * is secure, it will not display encrypted fields
   * 
  ...
  /**
   * Creates a String for the entity, overrides the default toString This method
   * is NOT secure, it WILL display encrypted fields
   * 
   * @return String corresponding to this entity
   */
