...
public class PrimaryKeyFinder extends Finder {
  ...
  public static GenericValue runFind(ModelEntity modelEntity, Map<String, Object> context, Delegator delegator, boolean useCache, boolean autoFieldMap, Map<FlexibleMapAccessor<Object>, Object> fieldMap, List<FlexibleStringExpander> selectFieldExpanderList) throws GeneralException {
    ...
    if (autoFieldMap) {
      ...
      // try a map called "parameters", try it first so values from here are
      // overridden by values in the main context
      ...
      // just get the primary keys, and hopefully will get all of them, if not
      // they must be manually filled in below in the field-maps
    ...
    try {
      ...
      // make sure we have a full primary key, if any field is null then just
      // log a warning and return null instead of blowing up
