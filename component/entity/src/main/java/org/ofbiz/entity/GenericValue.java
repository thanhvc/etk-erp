...
public class GenericValue extends GenericEntity implements Reusable {
  ...
  /**
   * This Map will contain the original field values from the database iff this
   * GenericValue came from the database. If it was made manually it will no
   * have this Map, ie it will be null to not take up memory.
   */
   ...
   * store, looking first in the global generic cache (for the moment this isn't
   * true, is same as EmbeddedCache variant)
   * 
   * @param relationName String containing the relation name which is the
   *          combination of relation.title and relation.rel-entity-name as
   *          specified in the entity XML definition file
  ...
  public List<GenericValue> getRelatedEmbeddedCache(String relationName) throws GenericEntityException {
    if (relatedCache == null)
      relatedCache = FastMap.newInstance();
  ...
  public GenericValue getRelatedOneEmbeddedCache(String relationName) throws GenericEntityException {
    if (relatedOneCache == null)
      relatedOneCache = FastMap.newInstance();
  ...
  /**
   * Get the named Related Entity for the GenericValue from the persistent store
   * and filter it, looking first in the global generic cache (for the moment
   * this isn't true, is same as EmbeddedCache variant)
   * 
   * @param relationName String containing the relation name which is the
   *          combination of relation.title and relation.rel-entity-name as
   *          specified in the entity XML definition file
  ...
  /**
   * Get the named Related Entity for the GenericValue from the persistent store
   * and filter it, looking first in a cache associated with this entity which
   * is destroyed with this ValueObject when no longer used.
   * 
   * @param relationName String containing the relation name which is the
   *          combination of relation.title and relation.rel-entity-name as
   *          specified in the entity XML definition file
  ...
  /**
   * Get the named Related Entity for the GenericValue from the persistent store
   * and order it, looking first in the global generic cache (for the moment
   * this isn't true, is same as EmbeddedCache variant)
   * 
   * @param relationName String containing the relation name which is the
   *          combination of relation.title and relation.rel-entity-name as
  ...
  /**
   * Get the named Related Entity for the GenericValue from the persistent store
   * and order it, looking first in a cache associated with this entity which is
   * destroyed with this ValueObject when no longer used.
   * 
   * @param relationName String containing the relation name which is the
   *          combination of relation.title and relation.rel-entity-name as
  ...
  /**
   * Checks to see if all foreign key records exist in the database. Will create
   * a dummy value for those missing when specified.
   * 
        ...
        if (count == 0) {
          if (insertDummy) {
            ...
            if (allFieldsSet) {
              ...
              // inherit create and update times from this value in order to
              // make this not seem like new/fresh data
              ...
              // set isFromEntitySync so that create/update stamp fields set
