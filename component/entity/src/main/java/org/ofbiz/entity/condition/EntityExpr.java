...
public class EntityExpr extends EntityCondition {
  ...
  public <L, R, LL, RR> void init(L lhs, EntityComparisonOperator<LL, RR> operator, R rhs) {
    ...
    // Debug.logInfo("new EntityExpr internal field=" + lhs + ", value=" + rhs +
    // ", value type=" + (rhs == null ? "null object" :
  ...
  public void checkCondition(ModelEntity modelEntity) throws GenericModelException {
    // if (Debug.verboseOn()) Debug.logVerbose("checkCondition for entity " +
    // modelEntity.getEntityName(), module);
  ...
  public void checkRhsType(ModelEntity modelEntity, Delegator delegator) {
    ...
    if (delegator == null) {
      // this will be the common case for now as the delegator isn't available
    ...
    if (value instanceof EntityConditionSubSelect) {
      ...
      // make sure the type of keyFieldName of EntityConditionSubSelect matches
      // the field Java type
      try {
  ...
  public boolean equals(Object obj) {
    ...
    boolean isEqual = equals(lhs, other.lhs) && equals(operator, other.operator) && equals(rhs, other.rhs);
    // if (!isEqual) {
    // Debug.logWarning("EntityExpr.equals is false for: \n-this.lhs=" +
    // this.lhs + "; other.lhs=" + other.lhs +
  ...
  @Override
  public int hashCode() {
    return hashCode(lhs) + hashCode(operator) + hashCode(rhs);
  }
}
