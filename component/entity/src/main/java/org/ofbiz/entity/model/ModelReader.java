...
public class ModelReader implements Serializable {
    ...
    if (entityCache == null) { // don't want to block here
      synchronized (ModelReader.class) {
        ...
        if (entityCache == null) { // now it's safe
            ...
            if (curChild != null) {
              ...
              do {
                ...
                if ((isEntity || isViewEntity) && curChild.getNodeType() == Node.ELEMENT_NODE) {
                  ...
                  // put the view entity in a list to get ready for the second
                  // pass to populate fields...
                  if (isViewEntity)
          ...
          // all entity elements in, now go through extend-entity elements and
          // add their stuff
          ...
          // do a pass on all of the view entities now that all of the entities
          // have
          ...
          for (ModelViewEntity curViewEntity : tempViewEntityList) {
            ...
            for (ModelViewEntity.ModelMemberEntity mve : curViewEntity.getAllModelMemberEntities()) {
              ...
              if (me == null)
                throw new GenericEntityConfException("View " + curViewEntity.getEntityName() + " references non-existant entity: " + mve.getEntityName());
          ...
          for (String curEntityName : new TreeSet<String>(this.getEntityNames())) {
            ...
            if (curModelEntity instanceof ModelViewEntity) {
              // for view-entities auto-create relationships for all
              // member-entity relationships that have all corresponding fields
              // in the view-entity

            } else {
              // for entities auto-create many relationships for all type one
              // relationships

              // just in case we add a new relation to the same entity, keep in
              // a separate list and add them at the end
              ...
              while (relationsIter.hasNext()) {
                ...
                if (("one".equals(modelRelation.getType()) || "one-nofk".equals(modelRelation.getType())) && !modelRelation.isAutoRelation()) {
                  ...
                  if (relatedEnt != null) {
                    ...
                    // create the new relationship even if one exists so we can
                    // show what we are looking for in the info message
                    ...
                    // decide whether it should be one or many by seeing if the
                    // key map represents the complete pk of the relEntity
                    if (curModelEntity.containsAllPkFieldNames(curEntityKeyFields)) {
                      // always use one-nofk, we don't want auto-fks getting in
                      // for these automatic ones
                      newRel.setType("one-nofk");

                      // to keep it clean, remove any additional keys that
                      // aren't part of the PK
                    ...
                    if (existingRelation == null) {
                      ...
                      if (newRel.equals(existingRelation)) {
                        // don't warn if the target title+entity = current
                        // title+entity
   ...
   * current entityResourceHandlerMap Map, must be done whenever a manual change
   * is made to the entityResourceHandlerMap Map after the initial load to make
    ...
    while (ecIter.hasNext()) {
      ...
      if (UtilValidate.isNotEmpty(packageFilterSet)) {
        ...
        if (!foundMatch) {
          ...
          // " becuase it is not in the package set: " + packageFilterSet,
          // module);
          continue;
        }
      }
      if (UtilValidate.isNotEmpty(entityFilterSet) && !entityFilterSet.contains(entityName)) {
        // Debug.logInfo("Not including entity " + entityName +
        // " becuase it is not in the entity set: " + entityFilterSet, module);
        continue;
      }
   ...
   * name, characters are stripped from the beginning of the name until a valid
   * entity name is found. It is intended to be used to determine the entity
   * name from a relation name.
   * 
