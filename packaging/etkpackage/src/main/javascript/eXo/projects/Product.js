...
Product.prototype.addDependencies = function(project) {
  try {
    ...
    if (project.hasDependency()) {
      ...
      for ( var depNum = 0; depNum < list.size(); depNum++) {
        this.addDependencies(list.get(depNum));
      }
    }
  } catch (error) {
...
Product.prototype.getDependencyById = function(depId) {
  ...
  for ( var depNum = 0; depNum < arrDep.length; depNum++) {
    var project = arrDep[depNum];
...
Product.prototype.DeployTask = function(product, server, repos) {
  ...
  descriptor.execute = function() {
    ...
    var nbDeps = product.getDependencies().iterator();
    counter = 0;
    var missingDeps = 0;
    while (nbDeps.hasNext()) {
      dep = nbDeps.next();
      try {
    ...
    if (missingDeps > 0) {
       eXo.System.error("Failed to find " + missingDeps + " files");
       eXo.System.error("!!! Please add them in the pom.xml (<dependencies> declarations are generated in this log file) !!!");
       throw("Error while deploying the project : " + product.name) ;
    }
