...
Project.prototype.setDeployName = function(deployName) {
	if(deployName != null) this.deployName = deployName;
	return this;
}
...
Project.prototype.extractTo = function(repository, dir, ignore) {
  //for( var i = 0; i < repository.length; i++) {
    try {
      var surl = repository[0] + "/" + this.relativePath;
      var url = new java.net.URL(surl);      
      eXo.System.info("PATCH", "Fetching patch at " + repository[0] + "/" + this.relativePath);
      //TODO 
      ...
      is.close() ;
        ...
        eXo.System.info("ERROR", "Artifact " + this.artifact + " not found in local maven repo, please add it in your pom.xml.");
        eXo.System.info("DEBUG", "<dependency><groupId>" + this.groupId + "</groupId><artifactId>" + this.artifactId + "</artifactId><version>${" + this.groupId +".version}</version><type>" + this.extension + "</type></dependency><!-- " + this.version + "-->") ;
        throw("Error missing dependency : " + this.artifactId);
    }
}

Project.prototype.deployTo = function(repository, server) {
  // EXP-Use only the local repo
  // for(var i = 0; i < repository.length; i++) {
      var surl = repository[0] + "/" + this.relativePath;
      var url = new java.net.URL(surl);
      ...
      out.close();
      is.close() ;
      eXo.System.info("DEBUG", fileName); 
      return;
}
