...
var version = "trunk";
var workflow = new Workflow("bonita", version, "workflow");
...
if (update_) {
  ...
  if ("all" != exclude_) {
    for ( var moduleNum = 0; moduleNum < product.dependencyModule.length; moduleNum++) {
      var module = product.dependencyModule[moduleNum];
...
if (deployServers != null && !deployServers.isEmpty()) {
  ...
  while (serv.hasNext()) {
    ...
    if (database != null) {
      ...
      //Gatein still don't require a database configure task. In addition, the database 
      //configure task of portal 2.5.x is incompatible with Gatein.
...
for ( var taskNum = 0; taskNum < tasks.size(); taskNum++) {
